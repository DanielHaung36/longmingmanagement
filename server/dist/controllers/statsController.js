...
class StatsController {
    ...
    static async getDashboardStats(req, res) {
        try {
            ...
            // 并发查询所有统计数据
            const [totalProjects, activeProjects, totalTasks, activeTasks, pendingApprovals, myTasks, tasksByJobType, // 修改：改为按 task.jobType 分组，因为 jobType 只在 task 级别定义
            tasksByStatus, tasksByPriority, recentActivitiesRaw,] = await Promise.all([
                // 项目总数
            ...
            const stats = {
                ...
                tasksByJobType: tasksByJobType.reduce((acc, item) => {
            ...
            res.status(200).json(responseBuilder_1.ResponseBuilder.success(stats, '仪表盘统计查询成功'));
        }
        catch (error) {
            logger_1.logger.error('仪表盘统计查询失败', { error: error.message });
            res.status(500).json(responseBuilder_1.ResponseBuilder.error(error.message, 'DASHBOARD_STATS_FAILED'));
        }
    }
    /**
     * 任务统计
    ...
    static async getTaskStats(req, res) {
        try {
            ...
            // 查询统计数据
            const [totalTasks, tasksByStatus, tasksByPriority, completionRate, avgEstimatedHours, avgActualHours,] = await Promise.all([
                // 总任务数
            ...
            res.status(200).json(responseBuilder_1.ResponseBuilder.success(stats, '任务统计查询成功'));
        }
        catch (error) {
            logger_1.logger.error('任务统计查询失败', { error: error.message });
            res.status(500).json(responseBuilder_1.ResponseBuilder.error(error.message, 'TASK_STATS_FAILED'));
        }
    }
    /**
     * 项目统计
    ...
    static async getProjectStats(req, res) {
        try {
            ...
            projectsByStatus, projectsByApprovalStatus, avgTasksPerProject, topClients,] = await Promise.all([
                // 总项目数
            ...
            const stats = {
                overview: {
                    total: totalProjects,
                    avgTasksPerProject: avgTasksPerProject.length > 0
                ...
                byApprovalStatus: projectsByApprovalStatus.reduce((acc, item) => {
            ...
            res.status(200).json(responseBuilder_1.ResponseBuilder.success(stats, '项目统计查询成功'));
        }
        catch (error) {
            logger_1.logger.error('项目统计查询失败', { error: error.message });
            res.status(500).json(responseBuilder_1.ResponseBuilder.error(error.message, 'PROJECT_STATS_FAILED'));
        }
    }
}
