...
const gdsService_1 = require("../services/gdsService");
...
router.get('/status', async (req, res) => {
    try {
        const available = await gdsService_1.GDSService.checkGDSAvailability();
        res.json({
            success: true,
...
router.post('/graph/create', async (req, res) => {
    try {
        ...
        const result = await gdsService_1.GDSService.createGraphProjection(graphName);
        res.json({
            success: true,
            data: result,
...
router.get('/graph/list', async (req, res) => {
    try {
        const graphs = await gdsService_1.GDSService.listGraphProjections();
        res.json({
            success: true,
            data: graphs
        });
...
router.delete('/graph/:graphName', async (req, res) => {
    try {
        ...
        await gdsService_1.GDSService.dropGraphProjection(graphName);
        res.json({
            success: true,
...
router.get('/similar-projects/:projectCode', async (req, res) => {
    try {
        ...
        const similar = await gdsService_1.GDSService.findSimilarProjects(projectCode, graphName, parseFloat(similarityCutoff) || 0.3);
        res.json({
            success: true,
            data: similar
        });
...
router.get('/communities', async (req, res) => {
    try {
        ...
        const communities = await gdsService_1.GDSService.detectCommunities(graphName);
        res.json({
            success: true,
            data: communities
        });
...
router.get('/communities/analyze', async (req, res) => {
    try {
        const { graphName } = req.query;
        const analysis = await gdsService_1.GDSService.analyzeCommunities(graphName);
        res.json({
            success: true,
            data: analysis
        });
...
router.get('/pagerank', async (req, res) => {
    try {
        ...
        const ranking = await gdsService_1.GDSService.calculatePageRank(graphName, nodeType);
        res.json({
            success: true,
            data: ranking
        });
...
router.get('/bridge-nodes', async (req, res) => {
    try {
        const { graphName } = req.query;
        const bridges = await gdsService_1.GDSService.findBridgeNodes(graphName);
        res.json({
            success: true,
            data: bridges
        });
...
router.get('/label-propagation', async (req, res) => {
    try {
        const { graphName } = req.query;
        const communities = await gdsService_1.GDSService.labelPropagation(graphName);
        res.json({
            success: true,
            data: communities
        });
...
router.get('/shortest-path', async (req, res) => {
    try {
        ...
        const path = await gdsService_1.GDSService.findShortestPath(sourceLabel, sourceName, targetLabel, targetName, graphName);
        res.json({
            success: true,
            data: path
        });
...
router.get('/recommend/:projectCode', async (req, res) => {
    try {
        const { projectCode } = req.params;
        const recommendations = await gdsService_1.GDSService.comprehensiveRecommendation(projectCode);
        res.json({
            success: true,
