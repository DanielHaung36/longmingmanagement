...
exports.mirrorFolderStructure = mirrorFolderStructure;
exports.verifyFolderStructure = verifyFolderStructure;
exports.generateVerifyReport = generateVerifyReport;
 ...
 * 检查两个文件是否具有相同的 hash
 */
function filesHaveSameHash(sourcePath, targetPath) {
    try {
 ...
 */
async function recursiveCreateFolders(sourceDir, targetDir, stats, createEmptyFiles = true) {
    try {
        // 创建目标文件夹
        ...
        for (const item of items) {
            ...
            // 规范化文件名
            const sanitizedName = (0, sanitizeFileName_1.sanitizeFileName)(item.name);
        ...
        stats.errors.push(`Error mirroring ${sourceDir} to ${targetDir}: ${errorMsg}`);
    }
}
/**
 * 递归验证文件夹结构
 */
async function recursiveVerifyStructure(sourceDir, targetDir, stats) {
    try {
        // 检查目标文件夹是否存在
        if (!fs.existsSync(targetDir)) {
        ...
        const errorMsg = error instanceof Error ? error.message : String(error);
        stats.errors.push(`Error verifying ${sourceDir} to ${targetDir}: ${errorMsg}`);
    }
}
/**
 * 主函数：镜像文件夹结构
 ...
 */
async function mirrorFolderStructure(sourceDir, targetDir, createEmptyFiles = true) {
    const stats = {
        foldersCreated: 0,
        filesCreated: 0,
        filesSkipped: 0,
 ...
 * 主函数：验证文件夹结构
 */
async function verifyFolderStructure(sourceDir, targetDir) {
    const stats = {
        totalFolders: 0,
        totalFiles: 0,
        missingFolders: [],
 ...
 * 生成验证报告
 */
function generateVerifyReport(stats) {
    let report = `\n╔════════════════════════════════════════════════════╗\n`;
    report += `║        文件夹结构验证报告                           ║\n`;
    report += `╚════════════════════════════════════════════════════╝\n\n`;
    report += `📊 验证统计:\n`;
...
//# sourceMappingURL=mirrorFolderStructure.js.map