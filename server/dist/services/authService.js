...
class AuthService {
    ...
    static async register(data) {
        ...
        // 1. 验证密码强度
        const passwordCheck = password_1.PasswordUtils.validateStrength(password);
        ...
        // 6. 发送欢迎邮件 (异步,不阻塞)
        email_1.EmailUtils.sendWelcomeEmail(email, username).catch(err => console.error('发送欢迎邮件失败:', err));
        // 7. 生成tokens
    ...
    static async login(data, ipAddress, userAgent) {
        ...
        // 2. 验证密码
        const isValid = await password_1.PasswordUtils.verify(password, user.password);
        if (!isValid) {
            throw new Error('用户名或密码错误');
        }
        // 3. 检查用户状态
    ...
    static async forgotPassword(email) {
        ...
        // 4. 发送重置邮件
        await email_1.EmailUtils.sendPasswordResetEmail(email, resetToken, user.username);
        return { message: '如果该邮箱已注册,您将收到重置密码的邮件' };
    }
    /**
     * 重置密码
    ...
    static async resetPassword(token, newPassword) {
        // 1. 验证密码强度
        const passwordCheck = password_1.PasswordUtils.validateStrength(newPassword);
        if (!passwordCheck.valid) {
            throw new Error(passwordCheck.message);
        }
        // 2. 查找token
        ...
        // 3. 加密新密码
        const hashedPassword = await password_1.PasswordUtils.hash(newPassword);
        // 4. 更新密码
        await prisma.users.update({
    ...
    static async changePassword(userId, currentPassword, newPassword) {
        ...
        // 2. 验证当前密码
        const isValid = await password_1.PasswordUtils.verify(currentPassword, user.password);
        if (!isValid) {
