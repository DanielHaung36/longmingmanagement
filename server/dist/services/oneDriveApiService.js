...
exports.OneDriveApiService = void 0;
...
class OneDriveApiService {
    ...
    static drivePrefix = ''; // e.g. "/sites/{siteId}/drive" or "/drives/{driveId}"
    // Azure AD 配置（从环境变量读取）
    static TENANT_ID = process.env.AZURE_TENANT_ID || '';
    static CLIENT_ID = process.env.AZURE_CLIENT_ID || '';
    static CLIENT_SECRET = process.env.AZURE_CLIENT_SECRET || '';
    // SharePoint 配置（优先）
    static SHAREPOINT_SITE_HOSTNAME = process.env.SHAREPOINT_SITE_HOSTNAME || 'longimagnetaustraliaptyltd.sharepoint.com';
    static SHAREPOINT_SITE_PATH = process.env.SHAREPOINT_SITE_PATH || '/sites/LongiAustralia';
    // OneDrive 根路径（相对于 OneDrive 根目录）
    static ONEDRIVE_BASE_PATH = process.env.ONEDRIVE_BASE_PATH || '03 Project Management';
    /**
     * 构建 API 路径（统一使用 SharePoint 站点文档库）
     */
    static buildItemPath(graphPath, suffix) {
    ...
    static async initialize() {
        ...
        if (!this.TENANT_ID || !this.CLIENT_ID || !this.CLIENT_SECRET) {
            logger_1.logger.warn('OneDrive API 未配置：缺少 Azure AD 凭据', {
        ...
        try {
            const tokenProvider = new ManualClientCredentialProvider(this.TENANT_ID, this.CLIENT_ID, this.CLIENT_SECRET);
            // 验证 token 获取
            ...
            try {
                ...
                logger_1.logger.info('SharePoint 站点已发现', { siteId: site.id });
            }
            catch (siteError) {
                logger_1.logger.error('SharePoint 站点发现失败，将无法访问文件', {
            ...
            logger_1.logger.info('OneDrive API 初始化成功（SharePoint 站点文档库）');
            return true;
        }
        catch (error) {
            logger_1.logger.error('OneDrive API 初始化失败', {
                error: error.message,
                stack: error.stack,
            });
            return false;
     ...
     */
    static convertToGraphPath(localPath) {
     ...
     * @returns 删除结果
     */
    static async deleteFolder(localPath) {
        try {
            // 1. 初始化客户端
            if (!await this.initialize()) {
                logger_1.logger.info('OneDrive API 未配置，跳过云端删除', { localPath });
            ...
            logger_1.logger.info('准备删除 OneDrive 云端文件夹', {
                localPath,
                graphPath,
            });
            // 3. 构建 API 路径
            ...
            try {
                ...
                if (checkError.statusCode === 404) {
                    logger_1.logger.info('OneDrive 云端文件夹不存在，无需删除', { graphPath });
                    return { success: true };
                }
                throw checkError;
            }
            ...
            logger_1.logger.info('OneDrive 云端文件夹删除成功', {
                localPath,
                graphPath,
            });
            return { success: true };
            ...
            logger_1.logger.error('OneDrive 云端文件夹删除失败', {
                localPath,
                error: error.message,
    ...
    static async createFolder(localPath) {
        try {
            if (!await this.initialize()) {
                logger_1.logger.info('OneDrive API 未配置，跳过云端创建', { localPath });
                return { success: true, skipped: true };
            }
            if (!this.client) {
                return { success: false, error: 'Graph Client 未初始化' };
            ...
            logger_1.logger.info('准备创建 OneDrive 云端文件夹', {
                localPath,
                graphPath,
                parentPath,
                folderName,
            ...
            logger_1.logger.info('OneDrive 云端文件夹创建成功', { graphPath });
            return { success: true };
        }
        catch (error) {
            // 如果文件夹已存在，视为成功
            if (error.statusCode === 409) {
                logger_1.logger.info('OneDrive 云端文件夹已存在', { localPath });
                return { success: true };
            }
            logger_1.logger.error('OneDrive 云端文件夹创建失败', {
                localPath,
                error: error.message,
                statusCode: error.statusCode,
            });
    ...
    static async renameFolder(oldPath, newName) {
        try {
            ...
            logger_1.logger.info('OneDrive 云端文件夹重命名成功', {
                oldPath,
                newName,
            });
            return { success: true };
            ...
            if (error.statusCode === 404) {
                logger_1.logger.info('OneDrive 云端文件夹不存在，跳过重命名', { oldPath });
                return { success: true };
            }
            logger_1.logger.error('OneDrive 云端文件夹重命名失败', {
                oldPath,
                newName,
                error: error.message,
            });
     ...
     */
    static async downloadFileAsBuffer(filePath) {
        try {
            if (!await this.initialize() || !this.client) {
                logger_1.logger.warn('OneDrive API 未配置，无法下载文件', { filePath });
                return null;
            }
            const graphPath = this.convertToGraphPath(filePath);
            const apiPath = this.buildItemPath(graphPath, '/content');
            logger_1.logger.debug('下载 OneDrive 文件', { filePath, graphPath });
            ...
            logger_1.logger.info('OneDrive 文件下载成功', { filePath, size: buffer.length });
            return buffer;
        }
        catch (error) {
            if (error.statusCode === 404) {
                logger_1.logger.info('OneDrive 文件不存在', { filePath });
                return null;
            }
            logger_1.logger.error('OneDrive 文件下载失败', {
                filePath,
                error: error.message,
                statusCode: error.statusCode,
            });
     ...
     * @returns 上传结果
     */
    static async uploadFile(filePath, content) {
        try {
            if (!await this.initialize() || !this.client) {
                logger_1.logger.info('OneDrive API 未配置，跳过上传', { filePath });
                return { success: true, skipped: true };
            }
            const graphPath = this.convertToGraphPath(filePath);
            const apiPath = this.buildItemPath(graphPath, '/content');
            logger_1.logger.debug('上传文件到 OneDrive', { filePath, graphPath, size: content.length });
            ...
            logger_1.logger.info('OneDrive 文件上传成功', { filePath, size: content.length });
            return { success: true };
        }
        catch (error) {
            logger_1.logger.error('OneDrive 文件上传失败', {
                filePath,
                error: error.message,
                statusCode: error.statusCode,
            });
     ...
     */
    static async listChildren(folderPath) {
        try {
            if (!await this.initialize() || !this.client) {
                return [];
            }
            ...
            logger_1.logger.debug('OneDrive listChildren 成功', { folderPath, count: items.length });
            return items;
        }
        catch (error) {
            if (error.statusCode === 404) {
                logger_1.logger.info('OneDrive 文件夹不存在', { folderPath });
                return [];
            }
            logger_1.logger.error('OneDrive listChildren 失败', {
                folderPath,
                error: error.message,
                statusCode: error.statusCode,
            });
     ...
     */
    static async ensureFolder(folderPath) {
        try {
            if (!await this.initialize() || !this.client) {
            ...
            logger_1.logger.error('OneDrive ensureFolder 失败', {
                folderPath,
                error: error.message,
            });
            return { success: false, error: error.message };
     ...
     */
    static async moveItem(itemPath, newParentPath, newName) {
        try {
            if (!await this.initialize() || !this.client) {
                return { success: true };
            }
            ...
            logger_1.logger.info('OneDrive moveItem 成功', { itemPath, newParentPath, newName });
            return { success: true };
        }
        catch (error) {
            if (error.statusCode === 404) {
                logger_1.logger.info('OneDrive 项不存在，跳过移动', { itemPath });
                return { success: true };
            }
            logger_1.logger.error('OneDrive moveItem 失败', {
                itemPath,
                error: error.message,
            });
            return { success: false, error: error.message };
     ...
     */
    static async folderExists(localPath) {
        try {
            if (!await this.initialize() || !this.client) {
                return false;
            }
            ...
            logger_1.logger.error('检查 OneDrive 文件夹存在性失败', { localPath, error: error.message });
            return false;
        }
    }
    /**
     ...
     */
    static async createShareLink(filePath) {
        try {
            if (!await this.initialize() || !this.client) {
                logger_1.logger.warn('OneDrive API 未配置，无法创建分享链接', { filePath });
                return null;
            }
            const graphPath = this.convertToGraphPath(filePath);
            ...
            logger_1.logger.info('OneDrive 分享链接创建成功', { filePath, url });
            return url;
        }
        catch (error) {
            logger_1.logger.error('创建 OneDrive 分享链接失败', { filePath, error: error.message });
            return null;
        }
    }
}
exports.OneDriveApiService = OneDriveApiService;
//# sourceMappingURL=oneDriveApiService.js.map