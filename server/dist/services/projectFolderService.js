...
exports.projectFolderService = exports.ProjectFolderService = void 0;
...
class ProjectFolderService {
     ...
     */
    static async ensureRootDirectory(rootPath, label) {
        if (!rootPath) {
            logger_1.logger.warn(`${label} 未配置，跳过根目录检查`);
            return;
        }
        try {
            if (label.includes('ONEDRIVE')) {
                // OneDrive 通过 Graph API 确保存在
                await oneDriveApiService_1.OneDriveApiService.ensureFolder(rootPath);
            }
            else {
                await fs.ensureDir(rootPath);
            }
            logger_1.logger.debug(`${label} 根目录就绪`, { rootPath });
        }
        catch (error) {
            logger_1.logger.error(`${label} 根目录创建失败`, { rootPath, error: error.message });
     ...
     * @returns 创建结果
     */
    static async createProjectFoldersById(projectId) {
        try {
            logger_1.logger.info("开始创建Project前两层文件夹", { projectId });
            // 1. 获取 Project 信息
            ...
            // 2. 创建本地文件夹结构
            await this.ensureRootDirectory(this.LOCAL_ROOT, 'LOCAL_PROJECT_ROOT');
            const localResult = await this.createLocalFolders(project.clientCompany, project.mineSiteName);
            // 3. 创建 OneDrive 文件夹结构（如果配置了）
            let oneDriveClientPath;
            let oneDriveMineSitePath;
            if (this.ONEDRIVE_ROOT) {
                await this.ensureRootDirectory(this.ONEDRIVE_ROOT, 'ONEDRIVE_PROJECT_ROOT');
                const oneDriveResult = await this.createOneDriveFolders(project.clientCompany, project.mineSiteName);
            ...
            logger_1.logger.error("创建Project文件夹失败", {
                projectId,
                error: error.message,
                stack: error.stack,
            });
     ...
     * 创建本地文件夹（前两层）
     */
    static async createLocalFolders(clientCompany, mineSiteName) {
        await this.ensureRootDirectory(this.LOCAL_ROOT, 'LOCAL_PROJECT_ROOT');
        // Sanitize folder names for OneDrive/Windows compatibility
        const sanitizedClientCompany = (0, folderNameSanitizer_1.sanitizeFolderName)(clientCompany);
        const sanitizedMineSiteName = (0, folderNameSanitizer_1.sanitizeFolderName)(mineSiteName);
        logger_1.logger.info("原始名称和清理后名称", {
        ...
        logger_1.logger.debug("创建客户文件夹", { localClientPath });
        ...
        if (!(await fs.pathExists(localClientInfoPath))) {
            ...
            logger_1.logger.info("创建客户信息文件夹", { localClientInfoPath });
        }
        // 3. 创建矿区文件夹
        await fs.ensureDir(localMineSitePath);
        logger_1.logger.debug("创建矿区文件夹", { localMineSitePath });
        ...
        if (!(await fs.pathExists(localProjectInfoPath))) {
            ...
            logger_1.logger.info("创建项目总览文件夹", { localProjectInfoPath });
        }
        return {
     ...
     */
    static async createOneDriveFolders(clientCompany, mineSiteName) {
        const sanitizedClientCompany = (0, folderNameSanitizer_1.sanitizeFolderName)(clientCompany);
        const sanitizedMineSiteName = (0, folderNameSanitizer_1.sanitizeFolderName)(mineSiteName);
        ...
        logger_1.logger.debug("创建OneDrive客户文件夹", { oneDriveClientPath });
        // 创建 "01 Client General info"
        const clientInfoPath = oneDriveClientPath.replace(/\\/g, "/") + "/01 Client General info";
        if (!(await oneDriveApiService_1.OneDriveApiService.folderExists(clientInfoPath))) {
            await oneDriveApiService_1.OneDriveApiService.createFolder(clientInfoPath);
            logger_1.logger.info("创建OneDrive客户信息文件夹", { clientInfoPath });
        }
        // 创建矿区文件夹
        await oneDriveApiService_1.OneDriveApiService.ensureFolder(oneDriveMineSitePath);
        logger_1.logger.debug("创建OneDrive矿区文件夹", { oneDriveMineSitePath });
        // 创建 "01 Project General Info"
        const projectInfoPath = oneDriveMineSitePath.replace(/\\/g, "/") + "/01 Project General Info";
        if (!(await oneDriveApiService_1.OneDriveApiService.folderExists(projectInfoPath))) {
            await oneDriveApiService_1.OneDriveApiService.createFolder(projectInfoPath);
            logger_1.logger.info("创建OneDrive项目总览文件夹", { projectInfoPath });
        }
        return {
     ...
     * 检查 Project 文件夹是否已创建
     */
    static async projectFoldersExist(projectId) {
        const project = await prisma.projects.findUnique({
            where: { id: projectId },
    ...
    static async renameProjectFolders(params) {
        ...
        const sanitizedOldClient = (0, folderNameSanitizer_1.sanitizeFolderName)(oldClientCompany);
        const sanitizedOldMine = (0, folderNameSanitizer_1.sanitizeFolderName)(oldMineSiteName);
        const sanitizedNewClient = (0, folderNameSanitizer_1.sanitizeFolderName)(newClientCompany);
        const sanitizedNewMine = (0, folderNameSanitizer_1.sanitizeFolderName)(newMineSiteName);
        // Old paths
        ...
        if (sanitizedOldClient === sanitizedNewClient &&
            ...
            logger_1.logger.info("🟡 客户与矿区名称未变化，跳过文件夹重命名。");
            return {
        ...
        if (!oldClientPathExists && !oldMineSitePathExists) {
            logger_1.logger.warn("⚠️ 未找到旧文件夹路径，可能是项目未创建或路径已变更。", {
                oldClientPath,
                oldMineSitePath,
            });
            return {
        ...
        try {
            ...
            if (sanitizedOldClient !== sanitizedNewClient && oldClientPathExists) {
                ...
                logger_1.logger.info("✅ 客户文件夹已重命名", {
                    oldClientPath,
                    newClientPath,
                });
            }
            ...
            if (sanitizedOldMine !== sanitizedNewMine && oldMineSitePathExists) {
                ...
                logger_1.logger.info("✅ 矿区文件夹已重命名", {
                    oldMineSitePath,
                    newMineSitePath,
                });
            }
            ...
            logger_1.logger.error("❌ 文件夹重命名失败", { error: err.message });
        }
        ...
        if (this.ONEDRIVE_ROOT) {
            try {
                ...
                if (sanitizedOldClient !== sanitizedNewClient) {
                    ...
                    const result = await oneDriveApiService_1.OneDriveApiService.renameFolder(oldOneDriveClientPath, sanitizedNewClient);
                    if (result.success) {
                        logger_1.logger.info("OneDrive 客户文件夹已重命名", { old: sanitizedOldClient, new: sanitizedNewClient });
                    }
                    else if (!result.skipped) {
                        logger_1.logger.warn("OneDrive 客户文件夹重命名失败", { error: result.error });
                ...
                if (sanitizedOldMine !== sanitizedNewMine) {
                    ...
                    const result = await oneDriveApiService_1.OneDriveApiService.renameFolder(oldOneDriveMineSitePath, sanitizedNewMine);
                    if (result.success) {
                        logger_1.logger.info("OneDrive 矿区文件夹已重命名", { old: sanitizedOldMine, new: sanitizedNewMine });
                    }
                    else if (!result.skipped) {
                        logger_1.logger.warn("OneDrive 矿区文件夹重命名失败", { error: result.error });
                        oneDriveSyncFailed = true;
                    }
                }
            }
            catch (err) {
                logger_1.logger.error("OneDrive 文件夹重命名失败", { error: err.message });
                oneDriveSyncFailed = true;
            }
        }
        return {
    ...
    static async deleteProjectFolders(params) {
        const { clientCompany, mineSiteName } = params;
        // Sanitize folder names
        const sanitizedClient = (0, folderNameSanitizer_1.sanitizeFolderName)(clientCompany);
        const sanitizedMine = (0, folderNameSanitizer_1.sanitizeFolderName)(mineSiteName);
        // Local paths
        ...
        if (await fs.pathExists(localMineSitePath)) {
            ...
            logger_1.logger.info('矿区文件夹已删除', { localMineSitePath });
        }
        ...
        if (await fs.pathExists(localClientPath)) {
            ...
            if (files.length === 0 || (files.length === 1 && files[0] === '01 Client General info')) {
                ...
                logger_1.logger.info('客户文件夹已删除（无其他矿区）', { localClientPath });
            }
        }
        ...
        if (this.ONEDRIVE_ROOT) {
            ...
            // 删除矿区文件夹
            await oneDriveApiService_1.OneDriveApiService.deleteItem(oneDriveMineSitePath);
            // 检查客户文件夹是否还有其他子项
            const children = await oneDriveApiService_1.OneDriveApiService.listChildren(oneDriveClientPath);
            if (children.length === 0 || (children.length === 1 && children[0].name === '01 Client General info')) {
                await oneDriveApiService_1.OneDriveApiService.deleteItem(oneDriveClientPath);
            }
        }
    }
}
exports.ProjectFolderService = ProjectFolderService;
// 导出单例实例
exports.projectFolderService = ProjectFolderService;
//# sourceMappingURL=projectFolderService.js.map