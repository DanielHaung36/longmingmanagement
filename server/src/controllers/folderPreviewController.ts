...
import { FolderPreviewService } from "../services/folderPreviewService";
 ...
 */
export const getTaskFolderContents = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const result = await FolderPreviewService.getTaskFolderContents(
      parseInt(taskId),
 ...
 */
export const getProjectFolderContents = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const result = await FolderPreviewService.getProjectFolderContents(
      parseInt(projectId),
      subfolder as string | undefined,
      folderType as "client" | "minesite" | undefined
    );
...
export const browseFolderContents = async (
  ...
  try {
    ...
    const result = await FolderPreviewService.getFolderContents(
  ...
  try {
    ...
    if (await fs.pathExists(filePathStr)) {
      ...
      fileStream.on("error", (error) => {
        logger.error("文件流读取错误", { filePath: filePathStr, error });
        if (!res.headersSent) {
          res.status(500).json({ success: false, message: "文件下载失败" });
        }
      });

      logger.info("本地文件下载成功", { filePath: filePathStr, fileName });
      return;
    }

    // 本地不存在，尝试从 OneDrive 下载
    const buffer = await OneDriveApiService.downloadFileAsBuffer(filePathStr);
    if (buffer) {
      res.setHeader("Content-Type", "application/octet-stream");
      res.setHeader("Content-Disposition", `attachment; filename="${encodeURIComponent(fileName)}"`);
      res.setHeader("Content-Length", buffer.length.toString());
      res.send(buffer);
      logger.info("OneDrive文件下载成功", { filePath: filePathStr, fileName });
      return;
    }
...
export const uploadFileToFolder = async (
  ...
  try {
    ...
    if (!task.oneDriveFolderPath) {
      logger.warn("尝试上传文件但Task未配置OneDrive路径", {
    ...
    let relativeSubPath = path.relative(
      normalizedOneDriveRoot,
    ...
    if (relativeSubPath.includes('..')) {
      logger.warn("上传路径不在任务的OneDrive目录下", {
        taskId: numericTaskId,
        targetPath,
        normalizedTargetPath,
      });
    ...
    await OneDriveApiService.ensureFolder(oneDriveTargetDir);
    const uploadResult = await OneDriveApiService.uploadFile(oneDriveFilePath, fileBuffer);
    if (!uploadResult.success && !uploadResult.skipped) {
      logger.warn("上传到OneDrive失败", { error: uploadResult.error });
    }

    // 清理临时文件
    await fs.remove(file.path).catch(() => {});

    let localSyncStatus: "SUCCESS" | "SKIPPED_NO_LOCAL_PATH" | "FAILED" = "SUCCESS";
    let localSyncError: string | undefined;
    let localFilePath: string | undefined;
    let expectedLocalPath: string | undefined;
 ...
 */
export const requestFileDelete = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    // 记录审计日志
    await AuditService.logFileDeleteRequest(
      fileId,
      file.fileName,
      reason,
      userId,
...
export const createFolder = async (
  ...
  try {
    ...
    if (await OneDriveApiService.folderExists(newOneDriveFolderPath)) {
      res.status(409).json({
        success: false,
        message: "文件夹已存在",
      });
    ...
    const createResult = await OneDriveApiService.createFolder(newOneDriveFolderPath);
    ...
    let localSyncStatus: "SUCCESS" | "SKIPPED_NO_LOCAL_PATH" | "FAILED" = "SUCCESS";
    let localSyncError: string | undefined;
    let newLocalFolderPath: string | undefined;
    ...
    await AuditService.logFolderCreate(
      task.id,
      sanitizedFolderName,
      newOneDriveFolderPath,
      sessionUserId,
      req.ip,
      req.headers["user-agent"] as string | undefined
    );
 ...
 */
export const approveFileDelete = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    if (approved) {
      ...
      try {
        ...
        logger.warn("删除物理文件失败（可能已被手动删除）", { error });
      }
      ...
      // 记录审计日志
      await AuditService.logFileDeleteApproval(
        fileId,
        file.fileName,
        true,
        comment,
...
export const createShareLink = async (
  ...
  try {
    ...
    const url = await OneDriveApiService.createShareLink(filePath as string);
    ...
    logger.error("创建分享链接失败", { path: req.query.path, error: error.message });
    res.status(500).json({ success: false, message: "创建分享链接失败" });
  }
};
