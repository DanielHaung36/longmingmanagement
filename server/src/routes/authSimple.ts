...
router.post('/register', async (req: Request, res: Response) => {
  try {
    ...
    // 调用注册服务（传递额外参数）
    const user = await CookieAuthService.register(username, email, password, realName, res, role, status, phone)
...
router.post('/login', async (req: Request, res: Response) => {
  try {
    ...
    // 执行登录（自动设置Cookie）
    const user = await CookieAuthService.login(username, password, res)
 ...
 * @access  Private
 */
router.post('/logout', cookieAuth, async (req: Request, res: Response) => {
  try {
    const sessionToken = CookieAuthService.extractToken(req.cookies)

    if (sessionToken) {
      await CookieAuthService.logout(sessionToken, res)
    }
...
router.get('/callback', async (req: Request, res: Response) => {
  try {
    ...
    let role = 'USER'
    if (rolesUpper.includes('ADMIN')) role = 'ADMIN'
    else if (rolesUpper.includes('MANAGER')) role = 'MANAGER'
    ...
    if (user) {
      ...
      const updates: any = {}
      if (user.cognitoId !== keycloakId) updates.cognitoId = keycloakId
    ...
    await CookieAuthService.createSessionForUser(user.id, user.username, user.email, role, res)
