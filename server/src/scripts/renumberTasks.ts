...
interface RenameOperation {
  ...
  newCode: string;
  title: string;
  oldOneDrivePath?: string;
  newOneDrivePath?: string;
  oldLocalPath?: string;
  newLocalPath?: string;
}
    ...
    for (const jobType of jobTypes) {
      ...
      for (const task of badTasks) {
        ...
        if (task.oneDriveFolderPath) {
          ...
          rename.newOneDrivePath = task.oneDriveFolderPath.replace(
            task.taskCode,
            newCode
          );
        }
        ...
        if (task.localFolderPath) {
          ...
          rename.newLocalPath = task.localFolderPath.replace(
            task.taskCode,
            newCode
          );
        }
...
function generateBashScript(renames: RenameOperation[]): string {
  let script = `#!/bin/bash
# 文件夹重命名脚本 (Linux/WSL)
...
function generatePowerShellScript(renames: RenameOperation[]): string {
