 ...
 */
export async function updateUsersData(prisma: PrismaClient, silent: boolean = false): Promise<void> {
  if (!silent) {
    console.log('\n🔧 开始更新用户数据...\n');
  }
    ...
    if (hillUser) {
      ...
      if (!silent) console.log('   ✅ 更新 hill@ljmagnet.com');
    }
    ...
    if (adelaUser) {
      ...
      if (!silent) console.log('   ✅ 更新 Adela@ljmagnet.com');
    }
    ...
    if (terranceUser) {
      ...
      if (!silent) console.log('   ✅ 更新 terrance@ljmagnet.com');
    }

    // 2. 添加新用户
    if (!silent) console.log('\n2️⃣ 添加新用户...');
    const defaultPassword = await bcrypt.hash('Longi@123', 10);
    ...
    for (const newUser of newUsers) {
      ...
      if (!exists) {
        ...
        if (!silent) console.log(`   ✅ 创建新用户: ${newUser.email}`);
      } else {
        if (!silent) console.log(`   ℹ️  用户已存在: ${newUser.email}`);
      }
    }
    ...
    // 手动过滤，不区分大小写
    const usersToDisable = allActiveUsers.filter(user =>
      !finalEmails.includes(user.email.toLowerCase()) &&
    ...
    for (const user of usersToDisable) {
      ...
      if (!silent) console.log(`   ⚠️  禁用用户: ${user.email}`);
    }
