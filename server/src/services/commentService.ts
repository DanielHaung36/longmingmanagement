...
// 只查2层：主评论 + 直接回复（性能最优）
const COMMENT_RELATION_INCLUDE: Prisma.commentsInclude = {
  users: {
    select: {
      id: true,
      username: true,
...
const getDisplayName = (user?: { realName?: string | null; username?: string | null } | null): string => {
...
export class CommentService {
  static async createComment(input: CreateCommentInput): Promise<any> {
    let parentComment: Awaited<ReturnType<typeof prisma.comments.findUnique>> | null = null;
    ...
    try {
      ...
      if (input.parentId) {
        ...
        if (
          parentComment.entityType !== input.entityType ||
      ...
      const mentionPayloads: MentionPayload[] = [];
      ...
      if (input.mentionedUserIds && input.mentionedUserIds.length > 0) {
        ...
        for (const mentionedUserId of validMentionIds) {
          ...
          if (mentionedUser) {
            ...
            WebSocketService.emitMentionNotification(mentionedUser.id, {
              commentId: comment.id,
      ...
      WebSocketService.emitCommentNotification({
        commentId: comment.id,
        username: getDisplayName(comment.users),
        entityType: input.entityType,
        entityId: input.entityId,
  ...
  static async getComments(
    ...
    try {
      logger.info('查询评论列表', { entityType, entityId, page, limit });
      ...
      logger.error('查询评论列表失败', { error: error.message });
      throw error;
    }
  }

  static async updateComment(input: UpdateCommentInput): Promise<any> {
    try {
      const { commentId, content, userId, mentionedUserIds } = input;

      logger.info('更新评论', { commentId, userId });
  ...
  static async deleteComment(commentId: number, userId: number): Promise<any> {
    try {
      logger.info('删除评论', { commentId, userId });
  ...
  static async getMentionedComments(
    userId: number,
    ...
    try {
      logger.info('查询被@提及的评论', { userId, page, limit });
      ...
      logger.error('查询被@提及的评论失败', { error: error.message });
      throw error;
    }
  }
  ...
  private static formatComment(comment: any): any {
    ...
    const mentions =
      comment.comment_mentions?.map((mention: any) => ({
    ...
    const replies = comment.other_comments?.map((reply: any) => this.formatComment(reply)) ?? [];
  ...
  private static async validateUsers(userIds: number[]): Promise<number[]> {
