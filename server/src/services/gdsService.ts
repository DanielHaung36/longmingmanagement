...
export class GDSService {
   ...
   * 创建图投影到内存
   */
  static async createGraphProjection(graphName: string = 'projectGraph'): Promise<any> {
    // 先检查图是否已存在，如果存在则删除
    try {
    ...
    return neo4jService.executeQuery(query, { graphName });
  }
   ...
   * 自动将客户、项目、矿物分成不同的社群
   */
  static async detectCommunities(graphName: string = 'projectGraph'): Promise<any> {
    const query = `
   ...
   * 统计每个社群的组成和特点
   */
  static async analyzeCommunities(graphName: string = 'projectGraph'): Promise<any> {
    // 先运行社群检测并写入属性
    await neo4jService.executeQuery(`
   ...
   * 找出最重要的客户、项目、矿物
   */
  static async calculatePageRank(
    graphName: string = 'projectGraph',
    nodeType?: string
  ): Promise<any> {
    const query = `
    ...
    return neo4jService.executeQuery(query, { graphName, nodeType });
  }
   ...
   * 识别连接不同社群的关键节点
   */
  static async findBridgeNodes(graphName: string = 'projectGraph'): Promise<any> {
    const query = `
   ...
   * 比 Louvain 更快，适合大规模图
   */
  static async labelPropagation(graphName: string = 'projectGraph'): Promise<any> {
    const query = `
   ...
   */
  static async comprehensiveRecommendation(projectCode: string): Promise<any> {
    ...
    try {
      ...
      const result = await neo4jService.executeQuery(query, { projectCode, graphName });
      ...
      // 确保清理资源
      try {
        await neo4jService.executeQuery(`CALL gds.graph.drop($graphName)`, { graphName });
      } catch (e) {
        // 忽略删除错误
      }
      throw error;
   ...
   * 删除图投影
   */
  static async dropGraphProjection(graphName: string): Promise<void> {
    try {
      await neo4jService.executeQuery(`
        CALL gds.graph.drop($graphName)
      `, { graphName });
   ...
   * 列出所有图投影
   */
  static async listGraphProjections(): Promise<any> {
    const query = `
      CALL gds.graph.list()
   ...
   * 检查 GDS 是否可用
   */
  static async checkGDSAvailability(): Promise<boolean> {
    try {
      await neo4jService.executeQuery(`RETURN gds.version() AS version`, {});
      return true;
    } catch (error) {
