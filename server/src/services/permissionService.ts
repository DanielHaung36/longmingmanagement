...
import { PrismaClient, UserRole, Permission, ResourceType, ActionType, PermissionScope } from '@prisma/client';
...
export async function getUserPermissions(userId: number): Promise<Permission[]> {
  try {
    ...
    const allPermissions = new Set([
      ...rolePermissions,
      ...userSpecificPermissions.map(p => p.permission_definitions.code as Permission),
    ]);
...
export async function hasPermission(
  ...
  try {
    ...
    if (resourceType && resourceId !== undefined) {
      ...
      if (isOwner) {
        const ownerPermissions = ['READ', 'UPDATE'] as const;
...
export async function grantUserPermission(
  ...
  try {
    ...
    if (!permissionDef) {
      throw new ApiError(404, `Permission ${permissionCode} not found`);
    }
...
export async function updateUserRole(
  ...
  try {
    ...
    if (!hasRoleManagePermission) {
      throw new ApiError(403, 'No permission to manage roles');
    }
