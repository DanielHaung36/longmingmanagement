...
import { sanitizeTaskFolderName, sanitizeFolderName } from '../utils/folderNameSanitizer';
...
export class TaskFolderService {
  ...
  // 模板文件夹根目录
  private static TEMPLATE_ROOT = process.env.TEMPLATE_ROOT || 'C:/Longi/Templates/Project Management Template';
  ...
  private static TEMPLATE_STRUCTURE_MAP: Record<string, {
    templateName: string;
    folders: string[];
  }> = {
    AC: {
  ...
  static async createTaskFolder(
    ...
    try {
      logger.info('开始创建Task文件夹', { taskId, taskCode, taskName, projectId, jobType });
      ...
      const thirdLayerName = sanitizeTaskFolderName(taskCode, taskName);

      logger.info('原始任务名称和清理后名称', {
      ...
      let oneDriveTaskPath: string | undefined;

      // 4. 创建 OneDrive 文件夹（通过 Graph API）
      if (this.ONEDRIVE_ROOT) {
        oneDriveTaskPath = path.join(
          this.ONEDRIVE_ROOT,
        ...
        logger.info('OneDrive Task文件夹创建成功', { oneDriveTaskPath });
      }

      // 5. 创建本地文件夹（运行时始终创建）
      if (this.LOCAL_ROOT) {
        localTaskPath = path.join(
          this.LOCAL_ROOT,
          project.clientCompany,
          project.mineSiteName,
          thirdLayerName
        ...
        logger.info('✅ 本地Task文件夹创建成功', { localTaskPath });
      }
   ...
   * 创建本地文件夹并复制模板（如果模板存在）
   */
  private static async createFolderWithTemplate(
    targetPath: string,
    jobType: string
  ): Promise<void> {
    const templateConfig = this.TEMPLATE_STRUCTURE_MAP[jobType];
    if (!templateConfig) {
      logger.warn(`未知的jobType: ${jobType}，创建空文件夹`);
    ...
    if (await fs.pathExists(templatePath)) {
      ...
      logger.info('模板复制成功', { templatePath, targetPath });
    } else {
      logger.warn('模板不存在，使用硬编码结构创建', { templatePath });
      await fs.ensureDir(targetPath);
   ...
   */
  private static async createOneDriveFolderWithTemplate(
    targetPath: string,
    jobType: string
  ): Promise<void> {
    // 创建主文件夹
    await OneDriveApiService.ensureFolder(targetPath);

    // 获取模板配置
    const templateConfig = this.TEMPLATE_STRUCTURE_MAP[jobType];
    if (!templateConfig) {
      logger.warn(`未知的jobType: ${jobType}，仅创建空文件夹`);
      return;
    }
    ...
    for (const folder of templateConfig.folders) {
      ...
      await OneDriveApiService.ensureFolder(subFolderPath);
    }

    logger.info(`OneDrive模板子文件夹创建完成: ${templateConfig.folders.length} 个`);
  }
   ...
   */
  private static async createTemplateFoldersFromConfig(
    basePath: string,
    folders: string[]
  ): Promise<void> {
    for (const folder of folders) {
      ...
      logger.debug('创建模板子文件夹', { folderPath });
    }
    logger.info(`成功创建 ${folders.length} 个模板子文件夹`);
  }
  ...
  static async renameTaskFolder(
    ...
    try {
      ...
      if (await fs.pathExists(oldLocalPath)) {
        ...
        logger.info('本地Task文件夹重命名成功', { oldLocalPath, newLocalPath });
      }
      ...
      if (task.oneDriveFolderPath) {
        ...
        const renameResult = await OneDriveApiService.renameFolder(oldOneDrivePath, newFolderName);
        if (renameResult.success) {
          logger.info('OneDrive Task文件夹重命名成功', { oldOneDrivePath, newOneDrivePath });
        } else if (!renameResult.skipped) {
          logger.warn('OneDrive Task文件夹重命名失败', { error: renameResult.error });
        }
      }
      ...
      logger.error('重命名Task文件夹失败', { taskId, error: error.message });
  ...
  static async deleteTaskFolder(
    ...
    try {
      ...
      if (task.oneDriveFolderPath) {
        const cloudDeleteResult = await OneDriveApiService.deleteFolder(task.oneDriveFolderPath);
        if (cloudDeleteResult.success) {
          if (cloudDeleteResult.skipped) {
            logger.info('OneDrive API 未配置，跳过云端删除', { path: task.oneDriveFolderPath });
          } else {
            logger.info('OneDrive 云端文件夹已通过 API 删除', { path: task.oneDriveFolderPath });
          }
        } else {
          logger.warn('OneDrive 云端删除失败，继续删除本地', {
      ...
      if (task.localFolderPath && (await fs.pathExists(task.localFolderPath))) {
        ...
        logger.info('本地项目数据目录已删除', { path: task.localFolderPath });
      }

      return { success: true };
    } catch (error: any) {
      logger.error('删除Task文件夹失败', { taskId, error: error.message });
      return { success: false, error: error.message };
    }
  }
   ...
   * @returns 是否存在
   */
  static async taskFolderExists(taskId: number): Promise<boolean> {
    const task = await prisma.tasks.findUnique({
      where: { id: taskId },
   ...
   * @returns 文件列表
   */
  static async getTaskFolderContents(taskId: number): Promise<{
    success: boolean;
    files?: string[];
    error?: string;
  }> {
    try {
      ...
      logger.error('获取Task文件夹内容失败', { taskId, error: error.message });
      return { success: false, error: error.message };
    }
  }
}
