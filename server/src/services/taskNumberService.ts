...
export class TaskNumberService {
  /**
   * 生成任务编号
   *
   ...
   */
  static async generateTaskCode(jobType: string): Promise<string> {
    try {
      logger.info("开始生成TaskCode", { jobType });
      ...
      if (tasks.length > 0) {
        ...
        if (sequences.length > 0) {
          ...
          logger.info("找到最大序号", { maxSequence, nextSequence: sequence });
        }
      }
      ...
      logger.info("TaskCode生成成功", { taskCode, sequence });
      return taskCode;
    } catch (error: any) {
      logger.error("生成TaskCode失败", { jobType, error: error.message });
   ...
   */
  static async taskCodeExists(taskCode: string): Promise<boolean> {
   ...
   */
  static async getNextSequence(jobType: string): Promise<number> {
    const tasks = await prisma.tasks.findMany({
      where: {
        taskCode: {
          startsWith: jobType,
   ...
   */
  static async generateBatchTaskCodes(
    jobType: string,
    count: number
  ): Promise<string[]> {
