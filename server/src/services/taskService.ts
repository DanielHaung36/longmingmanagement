...
import { TaskNumberService } from './taskNumberService';
...
export interface CreateTaskInput {
  title: string;
  description?: string;
  jobType: 'AC' | 'AP' | 'AQ' | 'AS' | 'AT';  // 必填：任务类型
  projectId: number;
  authorUserId: number;
  mineral?:string;
...
export class TaskService {
   ...
   * 5. 同步到 Excel（后续实现）
   */
  static async createTask(input: CreateTaskInput): Promise<any> {
    try {
      logger.info('开始创建Task', { input });
      ...
      const taskCode = await TaskNumberService.generateTaskCode(input.jobType);
      logger.info('taskCode生成成功', { taskCode });
      ...
      logger.info('Task数据库记录创建成功（DRAFT状态，等待审批）', { taskId: task.id, taskCode });
   ...
   * 更新 Task
   */
  static async updateTask(taskId: number, input: UpdateTaskInput): Promise<any> {
    try {
      logger.info('开始更新Task', { taskId, input });
      ...
      // 如果标题改变，重命名文件夹
      if (shouldRename) {
        logger.info('Task标题改变，重命名文件夹', {
          oldTitle: existingTask.title,
          newTitle: input.title,
        });

        const renameResult = await TaskFolderService.renameTaskFolder(
          taskId,
        ...
        if (!renameResult.success) {
          logger.warn('文件夹重命名失败', { error: renameResult.error });
        }
      }

      // 同步到 Excel（使用行号直接更新）
      if (updatedTask.approvalStatus === 'APPROVED' && updatedTask.excelRowNumber) {
        await TaskExcelSyncService.syncTaskUpdate(taskId);
      }

      logger.info('Task更新成功', { taskId });
      return updatedTask;
    } catch (error: any) {
      logger.error('更新Task失败', {
        taskId,
        input,
        error: error.message,
      });
    ...
    try {
      ...
      if (task.excelRowNumber) {
        ...
        try {
          ...
          logger.warn('Excel重建失败，但Task已删除成功', { taskId, error: excelError.message });
        }
      }

      logger.info('Task删除成功', { taskId, taskCode: task.taskCode });
   ...
   * 获取 Task 详情
   */
  static async getTaskById(taskId: number): Promise<any> {
    const task = await prisma.tasks.findUnique({
      where: { id: taskId },
      include: {
        projects: {
  ...
  static async getAllTasks(
    filters?: {
      search?: string;           // 全文搜索
      jobType?: string;          // 业务类型过滤
      mineral?: string;          // Mineral过滤
      status?: string;           // 工作流状态
      priority?: string;         // 优先级
      assignedUserId?: number;   // 分配给谁
      projectId?: number;        // 所属项目
      approvalStatus?: string;   // 审批状态
      dueRange?: 'dueThisWeek' | 'overdue'; // 到期范围
    },
    pagination?: {
      page?: number;
      limit?: number;
  ...
  static async updateTaskStatus(
    ...
    logger.info('Task状态更新成功', { taskId, status });
    return task;
  }
  ...
  static async createBatchTasks(
    projectId: number,
    tasks: Omit<CreateTaskInput, 'projectId'>[]
  ): Promise<any[]> {
    logger.info('开始批量创建Tasks', { projectId, count: tasks.length });
   ...
   */
  static async searchTasks(searchTerm: string, limit: number = 10): Promise<any[]> {
    try {
      logger.info('开始搜索Tasks', { searchTerm, limit });
      ...
      logger.info('搜索Tasks完成', { count: tasks.length });
   ...
   * 获取所有唯一的矿物类型
   */
  static async getUniqueMinerals(): Promise<string[]> {
    try {
      const tasks = await prisma.tasks.findMany({
        where: {
          mineral: {
      ...
      return minerals as string[];
    } catch (error: any) {
      logger.error('获取唯一矿物类型失败', {
        error: error.message,
      });
